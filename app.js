...
import {MASGraph, approximateLocalDomain, computeExtMAS, printEdge, substituteConsts, unfoldTemplates, UPPER_APPROX, LOWER_APPROX} from './Parser/multiAgentGraph.js';
...
const argv = yargs(hideBin(process.argv))
    ...
    .command({
        ...
        handler: (argv) => {
            let inputStr = fs.readFileSync(argv["input"]);
            let mg = new MASGraph();
...
function commandHandler(){
    ...
    if(argv.command === COMMAND.INFO){
        ...
        if(argv["params"]){
            ...
            // fs.writeFileSync(argv["output"], JSON.stringify(ld,null,4))
        }
        // mg.consumeConst();
